<script lang="ts">
  ...
  import {
    ...
    type TriageGraph, type TriageOption, type ScoredResult,
  } from '$lib/triage';
...
<style>
  ...
  .cta { background: var(--accent); color: white; border: none; padding: 0.8rem 1.3rem; border-radius: 10px; font-weight: 600; font-size: 0.95rem; cursor: pointer; }
  ...
  .match { display: flex; align-items: center; gap: 0.9rem; background: white; border: 1px solid var(--line); border-radius: 12px; padding: 0.9rem 1rem; margin-bottom: 0.6rem; text-decoration: none; color: var(--ink); }
